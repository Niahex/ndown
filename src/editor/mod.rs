...
// The EditorAreaRef struct and EditorAreaWidgetRefExt trait are automatically generated by #[derive(Widget)].
// We only need to implement methods on the generated EditorAreaRef.

impl EditorAreaRef {
    pub fn load_file(&self, cx: &mut Cx, filename: String) {
        if let Some(mut inner) = self.borrow_mut() {
            inner.load_file(cx, filename);
        }
    }
}
