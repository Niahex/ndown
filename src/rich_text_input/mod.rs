...
live_design! {
    ...
    pub RichTextInputBase = {{RichTextInput}} {}
    
    pub RichTextInputFlat = <RichTextInputBase> {
    ...
    pub RichTextInput = <RichTextInputFlat> {
        draw_bg: {
    ...
    pub RichTextInputGradientX = <RichTextInput> {
        draw_bg: {
    ...
    pub RichTextInputGradientY = <RichTextInputGradientX> {
        draw_bg: {
...
impl RichTextInput {
    ...
    fn handle_focus_lost(&mut self, cx: &mut Cx, scope_path: &HeapLiveIdPath, uid: WidgetUid) {
        ...
        cx.widget_action(uid, scope_path, RichTextInputAction::KeyFocusLost);
    }
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        match event.hits(cx, self.draw_bg.area()) {
            ...
            Hit::KeyFocus(_) => {
                ...
                cx.widget_action(uid, &scope.path, RichTextInputAction::KeyFocus);
            },
            Hit::KeyFocusLost(_) => {
                self.handle_focus_lost(cx, &scope.path, uid);
            }
            Hit::KeyDown(kev @ KeyEvent {
                ...
                if !did_move {
                    cx.widget_action(uid, &scope.path, RichTextInputAction::KeyDownUnhandled(kev));
                }
            }
            Hit::KeyDown(kev @ KeyEvent {
            ...
            Hit::KeyDown(kev @ KeyEvent {
                ...
                match self.move_cursor_up(cx, keep_selection) {
                    Ok(true) => { }
                    Ok(false) => cx.widget_action(uid, &scope.path, RichTextInputAction::KeyDownUnhandled(kev)),
            ...
            Hit::KeyDown(KeyEvent {
                ...
                cx.widget_action(uid, &scope.path, RichTextInputAction::Escaped);
            }
            Hit::KeyDown(KeyEvent {
                key_code: KeyCode::ReturnKey,
                modifiers: KeyModifiers {
            ...
            Hit::KeyDown(event) => {
                cx.widget_action(uid, &scope.path, RichTextInputAction::KeyDownUnhandled(event));
            }
            _ => {}
        }
    }
...
impl RichTextInputRef {
    ...
    pub fn returned(&self, actions: &Actions) -> Option<(String, KeyModifiers)> {
        for action in actions.filter_widget_actions_cast::<RichTextInputAction>(self.widget_uid()){
            if let RichTextInputAction::Returned(text, modifiers) = action {
    ...
    pub fn escaped(&self, actions: &Actions) -> bool {
        for action in actions.filter_widget_actions_cast::<RichTextInputAction>(self.widget_uid()){
            if let RichTextInputAction::Escaped = action {
                return true;
            }
        }
        false
    ...
    pub fn changed(&self, actions: &Actions) -> Option<String> {
        for action in actions.filter_widget_actions_cast::<RichTextInputAction>(self.widget_uid()){
            if let RichTextInputAction::Changed(text) = action{
                return Some(text);
            }
        }
        None
    ...
    pub fn key_down_unhandled(&self, actions: &Actions) -> Option<KeyEvent> {
        for action in actions.filter_widget_actions_cast::<RichTextInputAction>(self.widget_uid()){
            if let RichTextInputAction::KeyDownUnhandled(event) = action{
                return Some(event);
            }
        }
        None
    ...
    /// to a new `RichTextInputState` object.
    pub fn save_state(&self) -> RichTextInputState {
        if let Some(inner) = self.borrow() {
            RichTextInputState {
            ...
            RichTextInputState::default()
        }
    }

    /// Restores the internal state of this text input widget
    /// from the given `RichTextInputState` object.
    pub fn restore_state(&self, cx: &mut Cx, state: RichTextInputState) {
        if let Some(mut inner) = self.borrow_mut() {
